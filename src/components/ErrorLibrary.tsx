...
export const ErrorLibrary = ({ lang }: ErrorLibraryProps) => {
  ...
  const filteredErrors = DCS_ERRORS.filter(err => {
    ...
    const codeNormalized = err.code.toLowerCase().replace(/[-\s]/g, '');
    
    return (
      err.code.toLowerCase().includes(searchLower) ||
      codeNormalized.includes(searchNormalized) ||
      err.meaning[lang].toLowerCase().includes(searchLower) ||
      err.cause[lang].toLowerCase().includes(searchLower) ||
      err.solution[lang].toLowerCase().includes(searchLower) ||
      err.passengerMessage[lang].toLowerCase().includes(searchLower)
    );
  });
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredErrors.length > 0 ? (
          ...
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="py-20 text-center bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800"
          >
            ...
            <p className="text-zinc-400 dark:text-zinc-500 text-sm mt-1">{t.noResultsDesc || 'Arama kriterlerine uygun hata kodu bulunamadı.'}</p>
          </motion.div>
        )}
      </div>
    </div>
